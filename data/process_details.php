...
function process_service_detail($serviceid)
{
	...
	$details = array(
		...
		'State'                  => $current_state,
		'Duration'               => $duration,
		'DurationFmt'            => human_time(time() - $sd['last_state_change'], 2),
		'StateType'              => $statetype,
		'CurrentCheck'           => $current_check,
		'LastCheck'              => $last_check,
		'LastCheckFmt'           => human_time(time() - $sd['last_check'], 2),
		'NextCheck'              => $next_check,
		'NextCheckFmt'           => human_time(abs(time() - $sd['next_check']), 2),
		'LastStateChange'        => $last_state_change,
		'LastStateChangeFmt'     => human_time(time() - $sd['last_state_change'], 2),
